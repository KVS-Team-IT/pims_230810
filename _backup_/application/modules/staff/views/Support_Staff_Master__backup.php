...
<!--================================ PRESENT SERVICE DETAILS STARTS =================================-->
        <div class="background_div">
            <div class="row">
                ...
                <div class="form-group col-md-4 employment_cat" style="display:none;">
                <label for="" class="col-sm-12 col-form-label">Employment Category:<span class="reqd">*</span></label>
                <div class="col-sm-12">
                    <?php echo form_dropdown("employment_cat", array("" => "Select") + employment_cat(), isset($StfData->staff_cat) ? set_value('employment_cat', $StfData->staff_cat) : set_value('employment_cat'), ' id="employment_cat" class="form-control validate[required]" autocomplete="off" onchange=showDesigCat(this.value)'); ?>
                    <span class="error"><?php echo form_error('employment_cat'); ?></span>
                </div>
                </div>
                ...
                <div class="col-sm-12">
                    ...
                    <span class="error"><?php echo form_error('emp_strength'); ?></span>
                </div>
                </div>
            </div>
...
<script>
    ...
    $('#present_post_designation').on('typeahead:selected', function (evt, item) {
        ...
        if (present_post_designation != '') {
            processPresentDesignationAjaxCall(present_post_designation,present_post_cat);
        }
    });
    ...
    function processPresentDesignationAjaxCall(present_post_designation,present_post_cat) {
        $.ajax({
            url: base_url + 'autocomplete/get_designation',
            data: get_csrf_token_name + '=' + get_csrf_hash + '&designation=' + present_post_designation+ '&cat=' + present_post_cat,
            type: 'POST',
...
<script>
    ...
    function chkVacancy(){
        ...
        if(emp_typ==1){ //Contractual
            var emp_cat=$("#employment_cat").val();
            if(!(emp_cat) || emp_cat==''){
                alertify.error("Please Select Employement Category");
                return false;
            }
