...
<script id="family_more_template" type="x-tmpl-mustache">
    ...
    <div class="row" id="member-row-1{{random_exr_id}}" style="display:none" >
        ...
        <div class="col-sm-12">
        <?php echo form_input("dob[]",  '{{family_detail.dob}}', 'placeholder="dd-mm-yyyy" id="family_more_dob_datepicker{{random_exr_id}}"   class="datepicker-here form-control validate[required]" onchange="get_age({{random_exr_id}},family_more_dob_datepicker{{random_exr_id}}.value)" autocomplete="off"'); ?>
...
<script>
    ...
    if (family_details!='') {
        $.each(family_details, function (key, family_detail) {
            ...
            $("#family_more_dob_datepicker" + family_detail.id).val(formatDate(family_detail.dob)).trigger("change");
   ...
   /** get age on the basis of dob   */
    function get_age(random_exr_id,dob){
        ...
        var formatDOB = dob.split("-").reverse().join("-");
        var BirthDate = new Date(formatDOB);
        var today = new Date();
        var ageInMilliseconds = new Date(today - BirthDate);
