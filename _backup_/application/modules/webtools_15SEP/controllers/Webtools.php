...
class Webtools extends MY_Controller {
    ...
    public function Observation($enSno=null){
        ...
        if($ObsData = $this->input->post(null, true)) {
		    ...
		    $pdf->SetFont('freesans', '', 10);
                    ...
                    $html.='<input style="min-height:1px; font-size:12px;" type="checkbox" name="box" value="1" checked="checked" /><span style="font-weight: normal!important;">Whether writing assignment was given & collected back as image/pdf and correction done?</span><br>';
                    foreach(opt_yes_no() as $k=>$v){
                    ...
                    if ($ObsData['assignment_mode']==99){
                        $html.='<input style="min-height:1px; font-size:12px;" type="checkbox" name="box" value="1" checked="checked" /><span style="font-weight: normal!important;">Other</span><br>';
	...
	function SendObservedData(){
        ...
        $ObsData = $this->tools_model->fetch_observed_data();
        //show($ObsData);
        $excel_row = 2;
        $Sno=1;
