...
class Users extends MY_Controller {
    ...
    public function get_users() {
        ...
        $totalData = $this->user_model->get_all_users();
       
        $post_data = $this->input->post(null, true);
        $search = $this->input->post('search')['value'];
        $response = $this->user_model->get_all_users($limit, $start, $order, $dir, $search, $post_data);
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ++$serial;
                $nestedData['ID'] = $user->UID;
                $nestedData['UNAME'] = $user->UNAME;
                $nestedData['ROLE'] = $user->ROLE_ID;
                $nestedData['CAT'] = $user->CAT_ID;
                $nestedData['NAME'] = $user->NAME;
                $nestedData['RO'] = $user->RO;
                $nestedData['KV'] = $user->KV;
                $nestedData['ACT'] = $user->ACT;
                $nestedData['CPASS'] = $user->CPASS;
                $nestedData['MAIL'] = $user->MAIL;
    ...
    public function get_eusers() {
        ...
        $totalData = $this->user_model->get_all_eusers();
       
        $post_data = $this->input->post(null, true);
        $search = $this->input->post('search')['value'];
        $response = $this->user_model->get_all_eusers($limit, $start, $order, $dir, $search, $post_data);
    ...
    public function add_employee() {
        if ($PostData = $this->input->post(null, true)) {
            //show($PostData);
                $response = $this->user_model->set_employee($PostData);
                if ($response['status'] == 'success') {
	...
	public function subject_listsTGT(){
		subject_listsTGT_helper();
	}
	public function subject_listsPGT(){
		subject_listsPGT_helper();
	}
	public function chackvacancy(){
		if ($this->input->is_ajax_request()) {
			echo  chackvacancy_helper($_POST);die;
		}
	}
}
