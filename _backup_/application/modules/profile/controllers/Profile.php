...
if (!defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Profile extends MY_Controller {
        ...
        public function index($EmpPisId=null) {
            ...
            $view_data['AcdmData']['proE']=$this->profile_model->getProfessionalData($ExEc);
            //show($view_data['AcdmData']['proE']);
            $view_data['AcdmData']['proQ']= $this->profile_model->getProfessionalQualification($ExEc);
		...
		$retirement=date('01-m-Y',strtotime($retirement));
            ...
            //show($view_data['OtherTrainingData']);
            ...
            if ($EmpData = $this->input->post(null, true)) {
                $response = $this->profile_model->EmpAcceptance($EmpData);
        
            if ($response['status'] == 'success') {
                $this->session->set_flashdata('success', 'Profile Status Updated');
                redirect(base_url('employee-details/'. EncryptIt($EmpData['emp_id'])));
            } else{
