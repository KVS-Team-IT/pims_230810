...
class Staff_model extends CI_Model {
   ...
   public function getAllStaff($limit=null,$start=null,$col=null,$dir=null,$search=null,$post_data=null)
    ...
    public function getKvDetails($Kvid = null){
       ...
       (CASE 
        WHEN S.upto_class=1 THEN 'I'
        WHEN S.upto_class=2 THEN 'II'
        WHEN S.upto_class=3 THEN 'III'
        WHEN S.upto_class=4 THEN 'IV'
        WHEN S.upto_class=5 THEN 'V'
        WHEN S.upto_class=6 THEN 'VI'
        WHEN S.upto_class=7 THEN 'VII'
        WHEN S.upto_class=8 THEN 'VIII'
        WHEN S.upto_class=9 THEN 'IX'
        WHEN S.upto_class=10 THEN 'X'
        WHEN S.upto_class=11 THEN 'XI'
        WHEN S.upto_class=12 THEN 'XII' END) AS 'UPTO_CLASS',
        S.opening_year AS 'OPEN_YEAR',
        (CASE WHEN S.building=1 THEN 'PERMANENT' ELSE 'TEMPORARY' END) AS 'BUILDING',
        (CASE WHEN S.infrastructure=1 THEN 'YES' ELSE 'NO' END) AS 'INFRASTRUCTURE',
        S.section AS 'SECTION',
        (CASE 
        WHEN S.ccea_class=1 THEN 'I'
        WHEN S.ccea_class=2 THEN 'II'
        WHEN S.ccea_class=3 THEN 'III'
        WHEN S.ccea_class=4 THEN 'IV'
        WHEN S.ccea_class=5 THEN 'V'
        WHEN S.ccea_class=6 THEN 'VI'
        WHEN S.ccea_class=7 THEN 'VII'
        WHEN S.ccea_class=8 THEN 'VIII'
        WHEN S.ccea_class=9 THEN 'IX'
        WHEN S.ccea_class=10 THEN 'X'
        WHEN S.ccea_class=11 THEN 'XI'
        WHEN S.ccea_class=12 THEN 'XII' END) AS 'CCEA_CLASS',
        S.udisecode AS 'UDISE',
        (CASE WHEN S.science=1 THEN 'YES' ELSE 'NO' END) AS 'SCIENCE',
        (CASE WHEN S.commerce=1 THEN 'YES' ELSE 'NO' END) AS 'COMMERCE',
        (CASE WHEN S.arts=1 THEN 'YES' ELSE 'NO' END) AS 'ARTS',
        S.teaching_post AS 'CCEA_TECH',
        S.nonteaching_post AS 'CCEA_NONTECH'");
        $this->db->from('ci_schools S');
        $this->db->join('ci_regions R','R.id=S.reg_id','Left');
        $this->db->join('ci_regions Z','Z.id=S.zone','Left');
