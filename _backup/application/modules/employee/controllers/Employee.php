    ...
    */
    ...
    public function get_city()
    {
           if($this->input->post('state_id'))
            {
             $this->employee_model->fetch_city($this->input->post('state_id'));
            }
    }
    public function personal_details($EmpPisId=null) {
        $view_data['LogData'] = $this->common_model->LogMasterData();
        if(!empty($EmpPisId)){
    ...
    public function service_details($EmpPisId=null) {
        if(!empty($EmpPisId)){
            ...
            $HBD=date('d',strtotime($dob));
            
            $retirement = retirement_date( $view_data['dob']); 
            if($HBD=='01'){
                $retirement=date('01-m-Y',strtotime($retirement));
                $view_data['date_of_retirement'] = date_format(date_create($retirement."-1 month"), 't-m-Y');
            }else{
                $view_data['date_of_retirement'] = date_format(date_create($retirement), 't-m-Y');
            }
    ...
    public function training_section($EmpPisId=null) {
        ...
        $this->form_validation->set_rules('IS_TRAINED', 'Select Yes / No', 'required|integer|xss_clean|callback_regex_check');
            
            if($this->input->post('IS_TRAINED')==1){
                $this->form_validation->set_rules('TRN_COURSE[]', 'Course Type', 'required|xss_clean|callback_regex_check');
                $this->form_validation->set_rules('TRN_TOPIC[]', 'Tpoic ', 'required|xss_clean|callback_regex_check');
                $this->form_validation->set_rules('TRN_DESIG[]', 'Post Held', 'required|xss_clean|callback_regex_check');
                $this->form_validation->set_rules('TRN_SPAN[]', 'Duration', 'required|xss_clean');
                $this->form_validation->set_rules('TRN_SPELL[]', 'Spell ', 'required|xss_clean|callback_regex_check');
                $this->form_validation->set_rules('TRN_ROLE[]', 'Role ', 'required|xss_clean|callback_regex_check');
                /*
                foreach($this->input->post('TRN_SPELL[]') as $spl=>$splval){
                    if($splval==1){
                        $this->form_validation->set_rules('SPL_ONE_FROM[]', 'This Field', 'required|xss_clean|callback_regex_check');
                        $this->form_validation->set_rules('SPL_ONE_TO[]', 'This Field', 'required|xss_clean|callback_regex_check');
                        $this->form_validation->set_rules('SPL_ONE_VENUE[]', 'Venue', 'required|xss_clean|callback_regex_check');
                        $this->form_validation->set_rules('SPL_ONE_RO[]', 'This Field', 'required|xss_clean|callback_regex_check');
                    }else if($splval==2){
                        //$this->form_validation->set_rules('SPL_TWO_FROM[]', 'This Field', 'required|xss_clean|callback_regex_check');
                        //$this->form_validation->set_rules('SPL_TWO_TO[]', 'This Field', 'required|xss_clean|callback_regex_check');
                        //$this->form_validation->set_rules('SPL_TWO_VENUE[]', 'Venue', 'required|xss_clean|callback_regex_check');
                        //$this->form_validation->set_rules('SPL_TWO_RO[]', 'This Field', 'required|xss_clean|callback_regex_check');
                    }else if($splval==3){
                        $this->form_validation->set_rules('SPL_MUL_FROM[]', 'This Field', 'required|xss_clean|callback_regex_check');
                        $this->form_validation->set_rules('SPL_MUL_TO[]', 'This Field', 'required|xss_clean|callback_regex_check');
                        $this->form_validation->set_rules('SPL_MUL_VENUE[]', 'Venue', 'required|xss_clean|callback_regex_check');
                        $this->form_validation->set_rules('SPL_MUL_RO[]', 'This Field', 'required|xss_clean|callback_regex_check');
                    }
                }
                
                foreach($this->input->post('TRN_ROLE[]') as $rol=>$rolval){
                    if($rolval==4){
                        $this->form_validation->set_rules('TRN_GRADE[]', 'Grading or Rating', 'required|xss_clean|callback_regex_check');
                    }else if($rolval==3){
                        $this->form_validation->set_rules('TRN_CONDUCT[]', 'Course Conducted for', 'required|xss_clean|callback_regex_check');
                    }
                }
                 * 
                 */
            } // END OF IS TRAINED = 1(YES)
            /*
            $this->form_validation->set_rules('IS_EXMP', 'This Field', 'required|integer|xss_clean|callback_regex_check');
            if($this->input->post('IS_EXMP')==1){
                $this->form_validation->set_rules('EXEM_COURSE[]', 'Course Type', 'required|xss_clean|callback_regex_check');
                $this->form_validation->set_rules('EXEM_GROUND[]', 'Exemption Ground ', 'required|xss_clean|callback_regex_check');
                $this->form_validation->set_rules('EXEM_REASON[]', 'Reason of Exemption', 'required|xss_clean|callback_regex_check');
            }
            */
            if ($this->form_validation->run($this) !== FALSE) {
                ...
                if ($response['status'] == 'success') {
                    ...
                    $view_data['error_msg'] = isset($response['error']) ? $response['error'] : 'Data could not be Saved. Please try again';
                    redirect(base_url('training-details/'. EncryptIt( $emp_per_data['emp_id'] )));
                }
           }
        }
