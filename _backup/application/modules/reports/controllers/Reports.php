...
class Reports extends MY_Controller {
    ...
    public function index() {
		...
		if($role_id==1){
			$view = 'report/reports';
		}else if($role_id==2 && $this->role_category==9){
			$view = 'report/ex_reports';
		}else if($role_id==2 && $this->role_category==10){
			$view = 'report/ex_reports';
		}else if($role_id==2){
	...
	public function get_profile() {
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ...
                $nestedData['emp_acceptance'] = $user->emp_acceptance;
       ...
       $columns = array(
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ...
                $nestedData['PRE_SANCTION_POST'] = $user->PRE_SANCTION_POST;
                $nestedData['SANCTION_POST'] = $user->SANCTION_POST;
                $nestedData['IN_POSITION'] = $user->IN_POSITION;
                $nestedData['IN_SURPLUS'] = $user->IN_SURPLUS;
    ...
    public function ConsolidatedData() {
        ...
        if (!empty($ConData)) {
            ...
            foreach ($ConData as $cd) {
                ...
                $vac_teach=($cd->kvs_teach_post)-($cd->kv_teach_post);
                $vac_nonteach=($cd->kvs_nonteach_post)-($cd->kv_nonteach_post);
                ...
                $nestedData['kv_teach_vac'] = $vac_teach;
                $nestedData['kv_nonteach_vac'] = $vac_nonteach;
    ...
    function UnlockProfile(){
        if ($this->input->is_ajax_request()) {
            if (!empty($_POST['EmpId'])) {
                $EmpID=$_POST['EmpId'];
                $this->db->query("UPDATE ci_employee_details SET emp_acceptance='',emp_acceptancedate='' WHERE emp_code=$EmpID"); 
                echo true;
            }else{
                echo false;
            }
