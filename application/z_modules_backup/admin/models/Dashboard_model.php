...
class Dashboard_model extends CI_Model {
    ...
    public function TotalInPosADM(){
        ...
        $this->db->where('PS.present_status=', 1);
        $q = $this->db->get();
        if($q->num_rows() > 0) {
            return $q->row();
        } else {
    ...
    public function TotalInPosROKV($RoID=null){
        ...
        WHERE PS.present_status=1 AND PS.present_kvcode IN (SELECT `code` from ci_schools WHERE region_id=$RoID)";
    ...
    public function TotalInPosRO($RoID=null){
        ...
        FROM ci_present_service_details PS
        LEFT JOIN ci_designations DS ON(DS.id=PS.present_designationid)
        WHERE PS.present_status=1 AND PS.present_kvcode = (SELECT `code` from ci_regions WHERE id=$RoID)";
    ...
    public function ChartInPosROKV($RoID=null) {
        ...
        WHERE `PS`.present_status=1 AND `PS`.`present_unit`=$RoID AND `PS`.`present_place`=5
    ...
    public function ChartInPosRO($RoID=null) {
        ...
        WHERE `PS`.present_status=1 AND `PS`.`present_unit`=$RoID AND `PS`.`present_place`=3
    ...
    public function TotalInPosKV($KvID = null){
        ...
        FROM ci_present_service_details PS
        LEFT JOIN ci_designations DS ON(DS.id=PS.present_designationid)
        WHERE PS.present_status=1 AND PS.present_kvcode = (SELECT `code` from ci_schools WHERE id=$KvID)";
	...
	LEFT JOIN `ci_subjects` `SB` ON `PS`.`present_subject`=`SB`.`id`
        WHERE `PS`.present_status=1 AND `PS`.`present_school`=$KvID AND `PS`.`present_place`=5
    ...
    public function TotalInPosZT($ZtCODE=null){
        ...
        FROM ci_present_service_details PS
        LEFT JOIN ci_designations DS ON(DS.id=PS.present_designationid)
        WHERE PS.present_status=1 AND PS.present_kvcode = $ZtCODE";
    ...
    public function CIInPosZT($ZtCODE=null) {
        ...
        LEFT JOIN `ci_designations` `CD` ON `PS`.`present_designationid`=`CD`.`id`
	LEFT JOIN `ci_subjects` `SB` ON `PS`.`present_subject`=`SB`.`id`
        WHERE `PS`.present_status=1 AND `PS`.`present_kvcode`=$ZtCODE AND `PS`.`present_place`=4
        GROUP BY `PS`.`present_designationid`,`PS`.`present_subject`
        ORDER BY `CD`.`name` ASC";
    ...
    public function TotalInPosHQ($HqID=null){
        ...
        FROM ci_present_service_details PS
        LEFT JOIN ci_designations DS ON(DS.id=PS.present_designationid)
        WHERE PS.present_status=1 AND (PS.created_by = $HqID OR PS.updated_by = $HqID )";
        //show($PPHq);
    ...
    public function CIInPosHQ($HqCODE=null) {
        ...
        LEFT JOIN `ci_designations` `CD` ON `PS`.`present_designationid`=`CD`.`id`
	LEFT JOIN `ci_subjects` `SB` ON `PS`.`present_subject`=`SB`.`id`
        WHERE `PS`.present_status=1 AND `PS`.`present_kvcode`=$HqCODE AND `PS`.`present_place`=2";
        if($this->role_id==2 && $this->role_category ==9){ // HQ Establish Section- I
            $CIHq.="";
        }else if($this->role_id==2 && $this->role_category ==10){ // HQ Establish Section- I
            $CIHq.="";
        }else{
            $CIHq.=" AND `PS`.`present_section`=$this->role_category";
        }        
        if($this->role_id==2 && $this->role_category==9){ // HQ Establish Section- I
