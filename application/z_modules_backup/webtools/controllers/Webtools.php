...
class Webtools extends MY_Controller {
    ...
    public function Observation($enSno=null){
        ...
        if($ObsData = $this->input->post(null, true)) {
            ...
            if ($response == 1) {
                ...
                $Cc = isset($ObsData['emp_kvmail'])?$ObsData['emp_kvmail']:$ObsData['emp_mail'];
                ...
                $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, $EmpFor);
					...
					$html.='<br><br>';		
					$html.='<div style="width:100%; background-color:#014a69; color:#ffffff;font-size:12px;">
                            Suggestion (if any) ::&nbsp; </div><span style="padding:2px;">'.$ObsData['suggestions'].'</span>
                            ';
                    $html.='<br><br><hr>';
                    $html.='<table style="width:100%">
                        <tr>
                    ...
                    $this->mailerlib->pushMailAttach($Sub='Classroom Observation Data',$Msg='Madadm / Sir,<br> Please find the Classroom Observation Data file as an attachment to this mail.',$To= $this->email_id ,$Cc , $filename);
        ...
        $view_data['OD']= $this->tools_model->logObservedProfile();
        $view = 'Masterform';	
        $data = array(
            'title' => WEBSITE_TITLE . ' - ',
            'javascripts' => array(),
    ...
    public function All_Observed_Data(){
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
				...
				if(isset($user->overall_grading) && $user->overall_grading!='')
				{
					$nestedData['overall_grading'] = overall_grading($user->overall_grading);
				}else{
					$nestedData['overall_grading'] = 'N/A';
				}
    ...
    //echo 'Come'; die;
        ...
        $table_columns = array("SNO#","REGION","KV NAME","KV CODE","TEACHER NAME","TEACHER CODE","POST","SUBJECT",
            "OBSERVER NAME","DESIGNATION","OBSERVER REGION","OBSERVER UNIT","OVERALL GRADE","OBSERVED DATE");
        $column = 0;
        ...
        foreach($ObsData as $T){
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(10, $excel_row, $T->REGION);
            $object->getActiveSheet()->setCellValueByColumnAndRow(11, $excel_row, $T->UNIT);
            $object->getActiveSheet()->setCellValueByColumnAndRow(12, $excel_row, overall_grading($T->GRD));
            $object->getActiveSheet()->setCellValueByColumnAndRow(13, $excel_row,$T->ODT);
            $Sno++;
            $excel_row++;
        }
    ...
    public function ObservedUserList(){
        if ($PostData = $this->input->post(null, true)) {
            ...
            $this->form_validation->set_rules('observer_id', 'Observer Id', 'required|xss_clean|callback_regex_check');
            ...
            if ($this->form_validation->run($this) !== FALSE) {
                ...
                $response = $this->tools_model->addObserver($PostData);
                if ($response['status'] == 'success') {
                    $this->session->set_flashdata('success', 'User Added Successfully');
                    redirect(base_url('new-observed-user'));
                } else {
        ...
        $view_data['OBS']=$this->tools_model->ObserverList();
    ...
    public function get_observer() {
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($Role)) {
                ...
                $this->db->select("P.emp_id OBS_ID,CONCAT(E.emp_name,'  ',D.alias) OBS_NAME");
                $this->db->from('ci_present_service_details P');
                $this->db->join('ci_employee_details E','P.emp_id=E.emp_code','Left');
                $this->db->join('ci_designations D','P.present_designationid=D.id','Left');
                $this->db->where('P.present_status',1);
                $this->db->where('P.present_place',$Role);
                if($Role==2){
                    $DesigIds = array(19,20,21,22,23,24,25,26);
                    $this->db->where_in('P.present_designationid', $DesigIds);
                }
                if($Role==3){
                    $this->db->where('P.present_unit',$RoId);
                    $DesigIds = array(19,20,21,22,23,24,25,26);
                    $this->db->where_in('P.present_designationid', $DesigIds);
                }
                if($Role==5){
                    $this->db->where('P.present_unit',$RoId);
                    $this->db->where('P.present_school',$KvId);
                    $DesigIds = array(1,2,3,4);
                    $this->db->where_in('P.present_designationid', $DesigIds);
                }
                $this->db->order_by('E.emp_name ASC');
                $qry = $this->db->get();
                if ($qry->num_rows()) {
                    $Observers = $qry->result();
                    if (!empty($Observers)) {
                        foreach ($Observers as $key => $val) {
                           $options .='<option value="' . $val->OBS_ID . '">' . $val->OBS_NAME . '</option>';
                        }
                    }
                }
            }
    ...
    public function delete_observer(){
        if ($this->input->is_ajax_request()) {
            
            $ObsId  = $_POST['ObsId'];
            if($this->db->delete('ci_users', array('id' => $ObsId,'role_id'=>5,'role_category'=>3))){
                echo 1;
            }else{
                echo 0;
            } 
    ...
    public function delete_ro_observer(){
        if ($this->input->is_ajax_request()) {
            
            $ObsId  = $_POST['ObsId'];
            if($this->db->delete('ci_web_tools_observer', array('id' => $ObsId))){
                echo 1;
            }else{
                echo 0;
            } 
    ...
    public function AssignObserver(){
        if ($PostData = $this->input->post(null, true)) {
            $this->form_validation->set_rules('role_id', 'Role Id', 'required|xss_clean|callback_regex_check');
            if(!empty($this->input->post('region_id'))){
                $this->form_validation->set_rules('region_id', 'Region Id', 'required|xss_clean|callback_regex_check');
            ...
            if ($this->form_validation->run($this) !== FALSE) {
                //show($this->input->post());
                $response = $this->tools_model->AssignObserver($PostData);
                if ($response['status'] == 'success') {
                    $this->session->set_flashdata('success', 'Observer Assigned Successfully');
                    redirect(base_url('new-assign-observer'));
                } else {
                    $this->session->set_flashdata('error', 'Duplicate Assignment Found. Check form data and Try Again.');
                    redirect(base_url('new-assign-observer'));
                }
            }
        }
        $view_data['OBS']=$this->tools_model->AssignObserverList();
    ...
    public function AssignObserverUser() {
        if ($this->input->is_ajax_request()) {
            $options = '<option value="">Select</option>';
            $Role  = $_POST['r_id'];
            $RoId  = $_POST['ro_id'];
            ...
            if (!empty($Role)) {
                if($Role==2){ 
                    $RoId=33;
                    $Role_HQ=3;
                    $sql="SELECT A.U AS OBS_ID, CONCAT(E.emp_name,' - ', D.alias) OBS_NAME FROM
                    (SELECT 
                    id ID,role_id RO,role_category RC,region_id R,school_id S,username U,SUBSTRING(U.username,4) C
                    FROM ci_users U 
                    WHERE U.region_id=$RoId AND U.role_id=$Role_HQ AND U.role_category IN(0,1)) A

                    LEFT JOIN ci_present_service_details P ON(A.C=P.present_kvcode AND P.present_status=1)
                    LEFT JOIN ci_designations D ON (P.present_designationid=D.id)
                    LEFT JOIN ci_employee_details E ON(P.emp_id=E.emp_code)
                    WHERE P.emp_id<>'' AND P.present_designationid IN(19,20,21,22,23,24,25,26) GROUP BY E.emp_name ORDER BY D.id";
                }
               if($Role==3){ 
                    $sql="SELECT A.U AS OBS_ID, CONCAT(E.emp_name,' ( ', D.alias,' )') OBS_NAME FROM
                    (SELECT 
                    id ID,role_id RO,role_category RC,region_id R,school_id S,username U,SUBSTRING(U.username,4) C, 
                    (CASE WHEN SUBSTRING(U.username,1,2)='RO'  THEN 19 WHEN SUBSTRING(U.username,1,2)='AC'  THEN 23 ELSE '' END) D 
                    FROM ci_users U 
                    WHERE U.region_id=$RoId AND U.role_id=$Role AND U.role_category IN(0,1)) A
                    LEFT JOIN ci_designations D ON (A.D=D.id)
                    LEFT JOIN ci_present_service_details P ON(D.id=P.present_designationid AND A.R=P.present_unit AND P.present_status=1)
                    LEFT JOIN ci_employee_details E ON(P.emp_id=E.emp_code)
                    WHERE P.emp_id<>'' GROUP BY E.emp_name ORDER BY D.id";
                }
                if($Role==5){
                    $sql="SELECT A.U AS OBS_ID, CONCAT(E.emp_name,' ( ', D.alias,' )') OBS_NAME FROM
                    (SELECT 
                    id ID,role_id RO,role_category RC,region_id R,school_id S,username U,SUBSTRING(U.username,4) C, 
                    (CASE WHEN SUBSTRING(U.username,1,2)='KV'  THEN 1 WHEN SUBSTRING(U.username,1,2)='VP'  THEN 3 WHEN SUBSTRING(U.username,1,2)='HM'  THEN 4 WHEN SUBSTRING(U.username,1,2)='PG'  THEN 2 ELSE '' END) D 
                    FROM ci_users U 
                    WHERE U.region_id=$RoId AND U.role_id=$Role AND U.school_id=$KvId AND U.role_category IN(0,1,2)) A
                    LEFT JOIN ci_designations D ON (A.D=D.id)
                    LEFT JOIN ci_present_service_details P ON(D.id=P.present_designationid AND A.R=P.present_unit AND A.S=P.present_school AND P.present_status=1 )
                    LEFT JOIN ci_employee_details E ON(P.emp_id=E.emp_code)
                    WHERE P.emp_id<>'' GROUP BY E.emp_name ORDER BY D.id";
                }
				//echo $sql; die;
	...
	public function GetRegion(){
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $EmpData=region_lists($region_id);
			echo json_encode($EmpData[$region_id]);
	}
    }
	
	public function dashboard(){
		ini_set("memory_limit", "-1");
		$dashboardList = $this->tools_model->dashboardList();
		
		$dashboardVar = array();
		foreach($dashboardList as $key=>$val){
			$dashboardVar[$val->regionName][] = $val;
		}
		//echo "<pre>";print_r(count($dashboardVar));die('kkmmm');
		$totaldata = array();
		
		
		foreach($dashboardVar as $key=>$value){
			$principal = 0;
			$vp=0;
			$hm=0;
			$dc=0;
			...
			//if($key=='DELHI'){
				...
				if($val->obs_desig=='Principal'){
					$totaldata[$key]['Principal'] = ++$principal;
				}
				
				if($val->obs_desig=='Vice Principal'){
					$totaldata[$key]['vp'] = ++$vp;
				}
				if($val->obs_desig=='Head Master or Mistress'){
					$totaldata[$key]['hm'] = ++$hm;
				}
				if($val->obs_desig=='Deputy Commissioner'){
					$totaldata[$key]['dc'] = ++$dc;
					
				}
				if($val->obs_desig=='Assistant Commissioner'){
					$totaldata[$key]['ac'] = ++$ac;
				}
				$totaldata[$key]['total'] = $ac+$dc+$hm+$vp+$principal;
			}
		//}
		}
		//echo "<pre>";print_r($totaldata);die('kkmmm');
		//array_multisort( array_column( $totaldata, 'total' ), SORT_ASC );
		$totaldata = $this->array_sort($totaldata,'total',SORT_DESC);
		//ksort($totaldata);
		//echo "<pre>";print_r($totaldata);die('kkmmm');
		$view_data['dashboardData'] = $totaldata;
		$view = 'Dashboard';	
        $data = array(
            'title' => WEBSITE_TITLE . ' - ',
            'javascripts' => array(),
