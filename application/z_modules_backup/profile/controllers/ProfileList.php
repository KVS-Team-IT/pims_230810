...
class ProfileList extends MY_Controller {
    ...
    public function index() {
        ...
        $view = 'ProfileList';
        $data = array(
            'title' => WEBSITE_TITLE . ' - Registered Profile List',
            'javascripts' => array(),
            'style_sheets' => array(),
	...
	ini_set('memory_limit', '-1');
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ++$serial;
                $sub=($user->emp_sub && $user->emp_sub!='NA')?" (".$user->emp_sub.")":"";
                $nestedData['id'] = $user->id;
                $nestedData['emp_own'] = $user->emp_createdby;
                $nestedData['decode_id'] = EncryptIt($user->emp_code);
                $nestedData['emp_code'] = $user->emp_code;
                $nestedData['emp_post'] = $user->emp_desig.$sub;
                //$nestedData['emp_place'] = $user->emp_post_place;
                $nestedData['emp_dob'] = $user->emp_dob;
                $nestedData['is_admin'] = $this->session->userdata('user_id');
                ...
                $nestedData['emp_accept'] = $user->emp_accept;
