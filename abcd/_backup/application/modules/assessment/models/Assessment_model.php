...
class Assessment_model extends CI_model {
    ...
    public function get_class_wise_data($Class_ID,$SID) {
   ...
   public function FindStuKV() {
    ...
    public function KVAssessmentStatus($limit=null,$start=null,$col=null,$dir=null,$search=null,$post_data=null){
       ...
       // echo $table; die();
        if(func_num_args()==0)//this is used for getting total number of records
        {
            ...
            $this->db->from($table);
            //$this->db->where('region_id',$post_data['REGION_ID']); 
            $this->db->where('is_deleted','0'); 
            $qry=$this->db->get();
            return $qry->row()->total;
        }
        
        $this->db->select("SQL_CALC_FOUND_ROWS *,kv_id,name_of_kv,count(id) as totalStu",false);
        $this->db->from($table.' S');  
          
        if($post_data['REGION_ID']!='001'){   
            $this->db->where('S.region_id',$post_data['REGION_ID']); 
        } 
        $this->db->where('S.is_deleted','0');       
        $this->db->group_by('S.kv_id');       
        if($limit > 0){
           $this->db->limit($limit,$start);
        }
       $this->db->order_by($col, $dir); 
        if($search && !empty($search)) {  
            $this->db->where("CONCAT(S.name_of_kv) LIKE '%$search%' ");
        }
        $qry=$this->db->get(); 
      //echo $this->db->last_query(); die;		
        if($qry->num_rows()){
    ...
    { 
        ...
        //$this->db->from($table.' S');  
         $this->db->select("SQL_CALC_FOUND_ROWS S.*,S.id as sid,c2.student_id, c2.writing_hindi_ii",false);
        $this->db->from($table.' S');  
        $this->db->join('ci_students_class_2nd_rubrics c2','S.id=c2.student_id','Left');
        /*============filter for report=======================================  */ 
        $this->db->where('S.kv_id',$post_data['KV_ID']); 
		$this->db->where('S.is_deleted','0');  	
		 //$this->db->where('c2.is_deleted','0');
        if($limit > 0){
            $this->db->limit($limit,$start);
        }
       $this->db->order_by($col, $dir); 
    ...
    { 
        ...
        //$this->db->select("SQL_CALC_FOUND_ROWS *,DATE_FORMAT(S.created_on,'%d-%m-%Y %H:%i:%s') created_on",false);
        $this->db->select("SQL_CALC_FOUND_ROWS *, S.id as stu_id,S.name as stu_name,S.gender as stu_gender,S.category as stu_category,c2.writing_hindi_ii, (CASE 
                WHEN E.emp_title=1 THEN 'Sh.' 
                WHEN E.emp_title=2 THEN 'Smt.' 
                WHEN E.emp_title=3 THEN 'Ms.' END
            ) AS emp_title,",false);
    ...
    { 
        ...
        //$this->db->select("SQL_CALC_FOUND_ROWS *,DATE_FORMAT(S.created_on,'%d-%m-%Y %H:%i:%s') created_on",false);
        $this->db->select("SQL_CALC_FOUND_ROWS *,S.id as sid, c2.writing_hindi_ii",false);
        $this->db->from($table.' S');  
		$this->db->join('ci_students_class_2nd_rubrics c2','S.id=c2.student_id','Left');
         
        /*============filter for report=======================================  */ 
        $this->db->where('S.kv_id',$post_data['KV_ID']); 
        $this->db->where('S.teacher_id',$post_data['EMP_ID']); 
		$this->db->where('S.is_deleted','0');  		
        if($limit > 0){
            $this->db->limit($limit,$start);
        }
    ...
    { 
	 ...
	 public function student_is_exists($Class_ID,$SID) {
	  ...
	  public function get_student_by_id($Class_ID,$SID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
        ...
        $this->db->select('*,s.name as stu_name,s.id as  sid,R.`name` as region_name');
        $this->db->from($table.' s'); 		
        $this->db->join('ci_regions R','s.region_id=R.id','Left');
        $this->db->where('s.id', $SID);
         //echo $this->db->last_query(); die($table);
        return $this->db->get()->row();
    }
	
	 public function add_student($post_data, $Class_ID,$SID) {
        ...
        if (!empty($SID) && !empty($SID)) {
            ...
            $this->db->where('id', $SID);
            $qry = $this->db->update($table, $post_data);
            if ($qry) {
            ...
            $qry = $this->db->insert($table, $post_data);
            if ($qry) {
                $response['status'] = 'success';
                add_user_activity($this->auth_user_id, 'Added', json_encode($post_data), 'Student Added');
            } else {
                $response['status'] = 'error';
                $response['error'] = 'Some Error Occured';
            }
    ...
    public function getEmpPresentKV($emp_id){
         $this->db->select("present_kvcode",false);
        $this->db->from('ci_present_service_details');            
        $this->db->where('emp_id',$emp_id);  
        return $this->db->get()->row()->present_kvcode; 
          //echo $this->db->last_query(); die()
         
    }
	public function update_Class2ndyearEnd_term($post_data, $SID) {    
        ...
        if($kv_code==2402 || $kv_code==2440 || $kv_code==2434 || $kv_code==2444)
         {
            $post_data['kv_id']=$this->session->userdata['master_code'];
         }else{
            $post_data['kv_id']= $this->getEmpPresentKV($this->session->userdata('master_code')); 
        }
            ...
            if ($query->num_rows() > 0) {               
                ...
                $this->db->update($table, $post_data);
                 //echo $this->db->last_query(); die('update');
                $response['status'] = 'success';
            } else {
                $qry = $this->db->insert($table, $post_data);
                 //echo $this->db->last_query(); die('insert');
                if ($qry) {
                    $response['status'] = 'success';           
                } else {
                    $response['status'] = 'error';
            ...
            //die('end');
            add_user_activity($this->auth_user_id, 'Class 2nd YearEnd', json_encode($post_data), 'Assessment Update'); 
            return $response;
        }
    }

	public function update_yearEnd_term($post_data, $id) { 
        if (!empty($id) && !empty($id)) {
            $dataArray=array(
				'oral_converse'	=> $post_data['oral_converse'],
				'oral_talks'	=> $post_data['oral_talks'],
				'oral_recites'	=> $post_data['oral_recites'],
				'reading_participate'	=> $post_data['reading_participate'],
				'reading_uses'	=> $post_data['reading_uses'],
				'reading_sentences'	=> $post_data['reading_sentences'],
				'writing_words'	=> $post_data['writing_words'],
				'writing_convey'	=> $post_data['writing_convey'],
				'numeracy_count'	=> $post_data['numeracy_count'],
				'numeracy_read'	=> $post_data['numeracy_read'],
				'numeracy_addition'	=> $post_data['numeracy_addition'],
				'numeracy_observes'	=> $post_data['numeracy_observes'],
				'numeracy_units'	=> $post_data['numeracy_units'],
				'numeracy_recites'	=> $post_data['numeracy_recites'],
				'oral_hindi_frnd'	=> $post_data['oral_hindi_frnd'],
				'oral_hindi_convey'	=> $post_data['oral_hindi_convey'],
				'oral_hindi_story'	=> $post_data['oral_hindi_story'],
				'read_hindi_story'	=> $post_data['read_hindi_story'],
				'read_hindi_sound'	=> $post_data['read_hindi_sound'],
				'read_hindi_word'	=> $post_data['read_hindi_word'],
				'writing_hindi'	=> $post_data['writing_hindi'],
				'writing_hindi_drawing'	=> $post_data['writing_hindi_drawing'],
				);
            $this->db->where('id', $id);
            $qry = $this->db->update('ci_students', $dataArray);
            if ($qry) {
	...
	public function Update_EmpId_Selected_Data($empId, $kvsId, $rowId,$ClassID) {    
            ...
            foreach ($rowId as   $value) {
                $sqlQuery="UPDATE $table SET teacher_id='$empId'  WHERE id='$value' AND kv_id='$kvsId' AND region_id='$region_id'"; 
	...
	public function Update_Shift_Selected_Data($ShiftID, $rowId,$ClassID) {   
        if($ClassID=='1'){            
                $table='ci_students';
            }elseif ($ClassID=='2') {            
                $table='ci_students_backup_20june2022';
        ...
        $region_id=$this->session->userdata('region_id');
            foreach ($rowId as $value) {
                $sqlQuery="UPDATE $table SET kv_id='$ShiftID'  WHERE id='$value' AND region_id='$region_id'";
                $qry=$this->db->query($sqlQuery);               
            } 
			//$update_rows = array('kv_id' => $ShiftID); 
			//$this->db->where_in('id', $rowId );
			//$qry=$this->db->update('ci_students', $update_rows);	
            //echo $this->db->last_query(); die;			
			 if($qry){
                echo 1;
            }else{
    ...
    public function report_is_exists($id) {
      ...
      public function get_firstStep_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
        ...
        if($id!='001'){
            ...
            {     
                if($this->session->userdata('role_id')=='6'){ 
                    $T_ID= $this->session->userdata('master_code'); 
                    $where= "WHERE `region_id` = ".$id." and `teacher_id` = ".$T_ID." and `is_deleted` = '0' ORDER BY `id` ASC";
                 } else{
      ...
      public function get_page2A_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
...
SUM(CASE WHEN read_language_hindi='सरल वाक्यों को धाराप्रवाह पढ़ सकता  है एवं  उनकी  व्याख्या  कर सकता  है ।' THEN 1 ELSE 0 END) 'read_hindiD',
	  ...
	  public function get_YearEndOral_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndReading_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndWriting_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndNumeracy_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndHindi_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndNonSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
        ...
        if($id!='001'){
            ...
            { 
                if($this->session->userdata('role_id')=='6'){ 
                    $T_ID= $this->session->userdata('master_code'); 
                    $where= "WHERE `region_id` = ".$id." and `teacher_id` = ".$T_ID." AND `years_of_pre-schooling`='NIL' and `is_deleted` = '0' ORDER BY `id` ASC";
                 } else{
	   ...
	   public function get_YearEndNonNumSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndHindiNonSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndPreSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	   ...
	   public function get_YearEndPreNumSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndHindiPreSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndParent_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
        ...
        if($id!='001'){
            ...
            {   
                if($this->session->userdata('role_id')=='6'){ 
                    $T_ID= $this->session->userdata('master_code'); 
                    $where= "WHERE `region_id` = ".$id." and `teacher_id` = ".$T_ID." AND (`father_qualification` !='Not educated' AND `mother_qualification` !='Not educated') and `is_deleted` = '0' ORDER BY `id` ASC";
                 } else{
	   ...
	   public function get_YearEndParentSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndHindiParent_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndNonEdu_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	   ...
	   public function get_YearEndNonEduSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndHindiNonEdu_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndDis_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
        ...
        if($id!='001'){
			if($kv_id!=null || $kv_id!='')
            {
                if($this->session->userdata('role_id')=='6'){ 
                    $T_ID= $this->session->userdata('master_code'); 
                    $where= "WHERE `region_id` = ".$id." and `teacher_id` = ".$T_ID." AND `is_differently_abled` ='Yes' and `is_deleted` = '0' ORDER BY `id` ASC";
                 } else{
	   ...
	   public function get_YearEndDisSchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_YearEndHindiDis_data_by_id($id,$kv_id,$Class_ID) {
        if($Class_ID=='1'){            
            $table='ci_students';
        }elseif ($Class_ID=='2') {            
            $table='ci_students_backup_20june2022';
	  ...
	  public function get_EntryEdu_data_by_id($id,$kv_id) {
        if($id!='001'){
			if($kv_id!=null || $kv_id!='')
            {
                if($this->session->userdata('role_id')=='6'){ 
                    $T_ID= $this->session->userdata('master_code'); 
                    $where= "WHERE `region_id` = ".$id." and `teacher_id` = ".$T_ID." AND (`father_qualification` !='Not educated' OR `mother_qualification` !='Not educated') and `is_deleted` = '0' ORDER BY `id` ASC";
                 } else{
...
SUM(CASE WHEN read_language_hindi='सरल वाक्यों को धाराप्रवाह पढ़ सकता  है एवं  उनकी  व्याख्या  कर सकता  है ।' THEN 1 ELSE 0 END) 'read_hindi_DisD',
	  ...
	  //RTE QUERY
	 public function get_YearEndRTE_data_by_id($id,$kv_id,$Class_ID) {
        if($id!='001'){
			if($kv_id!=null || $kv_id!='')
            {
                if($this->session->userdata('role_id')=='6'){ 
                    $T_ID= $this->session->userdata('master_code'); 
                    $where= "WHERE `region_id` = ".$id." and `teacher_id` = ".$T_ID." AND `is_rte_quota` ='Yes' and `is_deleted` = '0' ORDER BY `id` ASC";
                 } else{
	  ...
	  public function get_YearEndRTESchooling_data_by_id($id,$kv_id,$Class_ID) {
        if($id!='001'){
			if($kv_id!=null || $kv_id!='')
            {
                if($this->session->userdata('role_id')=='6'){ 
	  ...
	  public function get_YearEndHindiRTE_data_by_id($id,$kv_id,$Class_ID) {
        if($id!='001'){
			if($kv_id!=null || $kv_id!='')
            {
                if($this->session->userdata('role_id')=='6'){ 
	  ...
	  public function get_EntryRTEHindi_data_by_id($id,$kv_id) {
		...
		public function deleterecord($id,$class_id)
	{
		if($class_id==1){
		$this->db->where('id',$id);
		$que=$this->db->delete('ci_students');
		}elseif($class_id==2){
		$this->db->where('id',$id);
		$que=$this->db->delete('ci_students_backup_20june2022');
		}
		if($que){
			return true;
		}else{
