...
class Tools_model extends CI_Model {
	...
	public function GetRecipient($ST = null){
        ...
        $this->db->where_in('D.id', ['5','6','7','8','9']);
        ...
        if($this->role_id==2){ // For KV Login
            ...
            if(sizeof($data)>0){
            $this->db->or_where("P.present_kvcode",$data->kv_code);   
            }
            $this->db->group_end();
        }
...
*/
    ...
    public function AssignObserverList(){
        $this->db->select("O.obs_userid OBS_ID,O.obs_name OBS_NAME,IFNULL(VR.`name`,'KVS HQ') OBS_REGION,  IFNULL(VS.`name`,'NA') OBS_SCHOOL,KR.`name` REGION,KS.`name` SCHOOL,DATE_FORMAT(O.obs_date,'%d-%m-%Y') OBS_DATE, (CASE WHEN O.obs_date >= CURDATE() THEN 'ACTIVE' ELSE 'SUSPEND' END) OBS_STATUS",false);
